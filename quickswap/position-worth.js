...
class PositionWorth {
  ...
  async _getAllUserPairsAndInitialReservesMap(args) {
    ...
    for (const poolInfo of Object.values(this.poolsInfo)) {
      ...
      const poolContract = new args.web3.eth.Contract(LP_ABI, poolInfo.poolAddress);
      const userBalanceBN = new BN(await poolContract.methods.balanceOf(args.address).call());

      const poolRewardsContract = new args.web3.eth.Contract(REWARDS_CONTRACT_ABI, poolInfo.rewardContractAddress);
      const userStakedBalanceBN = new BN(await poolRewardsContract.methods.balanceOf(args.address).call());

      if (userBalanceBN.isGreaterThan(0) || userStakedBalanceBN.isGreaterThan(0)) {
  ...
  async _getReserves(web3, poolContract, poolAddress) {
    ...
    if (!poolContract) {
      ...
      poolContract = new web3.eth.Contract(LP_ABI, poolAddress);
