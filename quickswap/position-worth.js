...
class PositionWorth {
  ...
  async onSubscribeForm(args) {
    const pairs = await this._getAllUserPairsAndInitialValues(args);
    return [
  ...
  async _getAllUserPairsAndInitialValues(args) {
    ...
    for (const poolInfo of Object.values(this.poolsInfo)) {
      ...
      const lpContract = new args.web3.eth.Contract(LP_ABI, poolInfo.poolAddress);
      const userBalance = await lpContract.methods.balanceOf(args.address).call();
      ...
      if (parseInt(userBalance) > 0) {
        ...
        initialValues[poolInfo.poolAddress] = reserves._reserve0;
  ...
  async _getLiquidity(args, lpToken) {
    const lpContract = new args.web3.eth.Contract(LP_ABI, lpToken);
    const reserves = await lpContract.methods.getReserves().call();
    return {
